...
#             print count
             if row[1] != '      nan':
                 sla[0,nn,0,4,count]=float(row[1])*10000
             if row[2] != '     nan':
                 sla[0,nn,0,1,count]=float(row[2])*100
             if row[3] != '     nan':
                 sla[0,nn,0,5,count]=float(row[3])*10000
             if row[4] != '     nan':
                 sla[0,nn,0,2,count]=float(row[4])*100
             if row[5] != '     nan':
                 sla[0,nn,0,6,count]=float(row[5])*10000
             if row[6] != '     nan':
                 sla[0,nn,0,7,count]=float(row[6])
             count=count+1
...
#             print count
             if row[1] != '      nan':
                 sla[0,nn,0,3,count]=((float(row[1]))**2)*10000
             if row[3] != '     nan':
                 sla[0,nn,0,0,count]=float(row[3])
             count=count+1
    return sla
rootgrp = Dataset("SSH.nc", "w", format="NETCDF4")
...
stats_sla = rootgrp.createVariable("stats_sla","f4",("time", "forecasts", "surface", "metrics", "areas",),fill_value=9999)
#stats_sla.Fill_Value = 9999
stats_sla.parameter = "Sea Level Anomaly" 
stats_sla.reference = "SLA along track from SLA TAC"
stats_sla.units = "cm"
sla=np.ones((1,1,1,8,17))
sla[sla == 1 ]=9999
fileanS=sys.argv[1]
fileanE=sys.argv[2]
#nna=sys.argv[3]
sla=fillvar(fileanS,fileanE,0)
stats_sla[:,:,:,:,:] = sla
