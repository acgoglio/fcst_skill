...
!get value of variable
      ...
      do k=1,lev
         do kk=1,prof
        theta=pot_tem(tem(k,kk),sal(k,kk),pressure(dpt(k,kk),lat(kk)))
        tem(k,kk)=theta
        enddo
      enddo
...
!!!! CONVERSION PRESSION TO DEPTH (non necessaria)
      ...
      do k=1,lev
         if (dpt(k,np).lt.5000) then
            !write(12,"(I5,I4,6f10.5,I8)")       &
            write(12,"(I8,I8,6f10.5)")       &
            indx, 1, lon(np), lat(np), dpt(k,np), rday - rinday, tem(k,np), 0
         endif
      enddo
      ...
      do k=1,lev
         if (dpt(k,np).lt.5000) then
            !write(12,"(I5,I4,6f10.5,I8)")       &
            write(12,"(I8,I8,6f10.5)")       &
            indx, 2, lon(np), lat(np), dpt(k,np), rday - rinday, sal(k,np), 0
         endif
      enddo
      endif
      enddo
