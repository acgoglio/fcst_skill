...
!---              SLA Observations             ---
      ...
      status = nf90_inq_varid (ncito, 'SLA', idslav)
      if (status /= nf90_noerr) call handle_err(status)
      ...
      status = nf90_inq_varid (ncito, 'SLApDAC', iddac)
      if (status /= nf90_noerr) call handle_err(status)
