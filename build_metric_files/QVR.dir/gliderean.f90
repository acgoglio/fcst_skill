...
! 2- Reading of Argo observations taken from routine ini_gld_oceanvar.f90     !
...
!-----------------------------------------------------------------------------!
 ...
 use obs_str_oceanvar
   ...
   implicit none
      ...
      integer, parameter:: znt=141, idt=1, ids=2, nonum=0, insid=3
...
!----------------------------------------------!
!  READ FILE WITH GLIDER OBSERVATIONS          !
!----------------------------------------------!

        open(411,file='GLIDER.dat',form='formatted',status='old',err=1234)
 ...
 if(gld%no.ne.0)then
   ...
   allocate ( gld%ino(gld%no), gld%par(gld%no), gld%val(gld%no), gld%err(gld%no))
...
! Read observations
        ...
        do k=1,gld%no
           gld%ino(k) = 1
           read(411,"(2I8,6f10.5)") gld%ino(k), gld%par(k), gld%lon(k)    &
                                  , gld%lat(k), gld%dpt(k)                &
                                  , gld%tim(k), gld%val(k), gld%err(k)      
           if ( k.eq.1 ) then
              print*,"first point"
           else if ( k.ne.gld%no) then
           if ( (gld%lon(k).ne.gld%lon(k-1)).or. &
              (gld%lat(k).ne.gld%lat(k-1)).or. &
              (gld%tim(k).ne.gld%tim(k-1)) ) then
              ...
              tab=startc+lenpr-1
              allocate ( matr(lenpr,5) )
              allocate ( temper(lenpr) )
              allocate ( salinita(lenpr), r(lenpr) )
              temper=-999999999
              salinita=-999999999
              matr(:,1)=gld%dpt(startc:tab)
              matr(:,2)=gld%val(startc:tab)
              matr(:,3)=gld%val(tab+1:endc)
!---
! Horizontal interpolation
              call inter_sys(trim(infile),gld%lon(k-1), &
                   gld%lat(k-1),xx,yy,depv,toc,soc)
!---
! Region 
...
! Writng results
              do kk=1,lenpr
                 if (matr(kk,4).gt.-10) then
                    write(12,11)nreg,',',idt,',',gld%lon(k-1),',', &
                       gld%lat(k-1),',',matr(kk,1),',',matr(kk,4),',', &
                       matr(kk,2),',',insid
                 endif
              enddo
              do kk=1,lenpr
                 if (matr(kk,5).gt.-10) then
                    write(12,11)nreg,',',ids,',',gld%lon(k-1),',', &
                       gld%lat(k-1),',',matr(kk,1),',',matr(kk,5),',', &
                       matr(kk,3),',',insid
                 endif
              enddo
           ...
           ! last profile of the GLIDER.dat
              endc=k
              lenpr=(endc-startc+1)/2
              tab=startc+lenpr-1
              allocate ( matr(lenpr,5) )
...
! Writng results
        ...
        enddo !end read observations
         ...
         write(12,11)nonum,',',nonum,',',nonum,',', &
                       nonum,',',nonum,',',nonum,',', &
                       nonum,',',insid   
      endif !end if statement gld%no.ne.0
      close(12)
