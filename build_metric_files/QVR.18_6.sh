...
#InDir=$1
WorkDir=/work/oda/ag15419/tmp/Ana_Fcst/testVALFOR_18_6/work/ #Da modificare
#LogDir=$3
OutDir=/work/oda/ag15419/tmp/Ana_Fcst/testVALFOR_18_6/out/ #Da modificare
...
MYE_DELIVERY2=/data/products/MFS/MFS_EAS6v8/ #Directory dove sono archiviati i file da utilizzare
...
#    bestest=`find -L $MYE_DELIVERY2/$diran -name {NRT_SYSNAME}*-${day}-a-T.nc`
    ...
    dayfc2=`python ${BinDir}/jday.py ${day} -1`
    dayfc3=`python ${BinDir}/jday.py ${day} -2`
    dayfc4=`python ${BinDir}/jday.py ${day} -3`
    dayfc5=`python ${BinDir}/jday.py ${day} -4`
    dayfc6=`python ${BinDir}/jday.py ${day} -5`
    dayfc7=`python ${BinDir}/jday.py ${day} -6`
    dayfc8=`python ${BinDir}/jday.py ${day} -7`
    dayfc9=`python ${BinDir}/jday.py ${day} -8`
    dayfc10=`python ${BinDir}/jday.py ${day} -9`
    # Da decidere la climatologia
    cl=`find -L ${VAL_WORKDIR}/QVR_DATA0/ -name ${NRT_SYSNAME}-20210301-2019${mm}01-a-T.nc`
    # Da modificare 95-116 in base a nome file utilizzati
    bestest=`find -L $MYE_DELIVERY2/$diran -name ${NRT_SYSNAME}*-${day}-a-T.nc`
    sm=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayb}-${day}-s-T.nc`
    fc1=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc1}-${day}-f-T.nc`
    ps1=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc1}-${day}-f-T.nc`
    fc2=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc2}-${day}-f-T.nc`
    ps2=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc2}-${dayfc2}-f-T.nc`
    fc3=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc3}-${day}-f-T.nc`
    ps3=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc3}-${dayfc3}-f-T.nc`
    fc4=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc4}-${day}-f-T.nc`
    ps4=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc4}-${dayfc4}-f-T.nc`
    fc5=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc5}-${day}-f-T.nc`
    ps5=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc5}-${dayfc5}-f-T.nc`
    fc6=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc6}-${day}-f-T.nc`
    ps6=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc6}-${dayfc6}-f-T.nc`
    fc7=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc7}-${day}-f-T.nc`
    ps7=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc7}-${dayfc7}-f-T.nc`
    fc8=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc8}-${day}-f-T.nc`
    ps8=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc8}-${dayfc8}-f-T.nc`
    fc9=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc9}-${day}-f-T.nc`
    ps9=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc9}-${dayfc9}-f-T.nc`
    fc10=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc10}-${day}-f-T.nc`
    ps10=`find -L $MYE_DELIVERY2/$dirfc -name ${NRT_SYSNAME}-${dayfc10}-${dayfc10}-f-T.nc`
    lcl=mfs_easO-20210301-2019${mm}01-a-T.nc
    # esempio di file finale mfs_easO-20220529-20220601-f-T.nc
    bbestest=mfs_easO-${ProcDay}-${day}-a-T.nc
    bsm=mfs_easO-${dayb}-${day}-s-T.nc
    bfc1=mfs_easO-$(echo `basename $fc1` | cut -d '-' -f2-)
    bfc2=mfs_easO-$(echo `basename $fc2` | cut -d '-' -f2-)
    bfc3=mfs_easO-$(echo `basename $fc3` | cut -d '-' -f2-)
    bfc4=mfs_easO-$(echo `basename $fc4` | cut -d '-' -f2-)
    bfc5=mfs_easO-$(echo `basename $fc5` | cut -d '-' -f2-)
    bfc6=mfs_easO-$(echo `basename $fc6` | cut -d '-' -f2-)
    bfc7=mfs_easO-$(echo `basename $fc7` | cut -d '-' -f2-)
    bfc8=mfs_easO-$(echo `basename $fc8` | cut -d '-' -f2-)
    bfc9=mfs_easO-$(echo `basename $fc9` | cut -d '-' -f2-)
    bfc10=mfs_easO-$(echo `basename $fc10` | cut -d '-' -f2-)
    bps1=mfs_easO-$(echo `basename $ps1` | cut -d '-' -f2-)
    bps2=mfs_easO-$(echo `basename $ps2` | cut -d '-' -f2-)
    bps3=mfs_easO-$(echo `basename $ps3` | cut -d '-' -f2-)
    bps4=mfs_easO-$(echo `basename $ps4` | cut -d '-' -f2-)
    bps5=mfs_easO-$(echo `basename $ps5` | cut -d '-' -f2-)
    bps6=mfs_easO-$(echo `basename $ps6` | cut -d '-' -f2-)
    bps7=mfs_easO-$(echo `basename $ps7` | cut -d '-' -f2-)
    bps8=mfs_easO-$(echo `basename $ps8` | cut -d '-' -f2-)
    bps9=mfs_easO-$(echo `basename $ps9` | cut -d '-' -f2-)
    bps10=mfs_easO-$(echo `basename $ps10` | cut -d '-' -f2-)
    Cmd="ln -s $cl ./${day}/${lcl}"
    echo $Cmd
    eval $Cmd
