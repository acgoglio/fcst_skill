...
#        RMS_T16=np.sqrt(np.true_divide(TEMPERATURE16[:,0,0,3,0],TEMPERATURE16[:,0,0,0,0], where=(TEMPERATURE16[:,0,0,0,0]!=0)))
        RMS_T16=np.sqrt(TEMPERATURE16[:,3,depth,3,0], where=(TEMPERATURE16[:,0,depth,0,0]!=0))
        STAT_T16.extend(RMS_T16)
        RMS_S16=np.sqrt(SALINITY16[:,3,depth,3,0], where=(SALINITY16[:,0,depth,0,0]!=0))
        STAT_S16.extend(RMS_S16)
        RMS_SLA16=np.sqrt(SLA16[:,3,0,3,0], where=(SLA16[:,0,0,0,0]!=0))
        STAT_SLA16.extend(RMS_SLA16)
        RMS_SST16=np.sqrt(SST16[:,3,0,3,0], where=(SST16[:,0,0,0,0]!=0))
        STAT_SST16.extend(RMS_SST16)
        OBS_T16.extend(TEMPERATURE16[:,3,depth,0,0])
        OBS_S16.extend(SALINITY16[:,3,depth,0,0])
        OBS_SLA16.extend(SLA16[:,3,0,0,0])
        OBS_SST16.extend(SST16[:,3,0,0,0])
        T16.extend(times16)
...
#Evaluates the time period
          ...
          RMS_T24=np.sqrt(TEMPERATURE24[:,3,depth,3,0], where=(TEMPERATURE24[:,0,depth,0,0]!=0))
          STAT_T24.extend(RMS_T24)
          RMS_S24=np.sqrt(SALINITY24[:,3,depth,3,0], where=(SALINITY24[:,0,depth,0,0]!=0))
          STAT_S24.extend(RMS_S24)
          RMS_SLA24=np.sqrt(SLA24[:,3,0,3,0], where=(SLA24[:,0,0,0,0]!=0))
          STAT_SLA24.extend(RMS_SLA24)
          RMS_SST24=np.sqrt(SST24[:,3,0,3,0], where=(SST24[:,0,0,0,0]!=0))
          STAT_SST24.extend(RMS_SST24)
          OBS_T24.extend(TEMPERATURE24[:,3,depth,0,0])
          OBS_S24.extend(SALINITY24[:,3,depth,0,0])
          OBS_SLA24.extend(SLA24[:,3,0,0,0])
          OBS_SST24.extend(SST24[:,3,0,0,0])
          T24.extend(times24)
...
ax.grid('on')
plt.title('RMSD TEMPERATURE 10-30m MEDSEA 3rd DAY FORECAST' ,fontsize=18)
ax.set_ylim([0.2,2.0])
...
plt.savefig(figdir+'TEMP_RMSD_10-30_3RD_MED_ma.jpg')
##
...
leg.get_frame().set_alpha(0.3)
ax.grid('on')
plt.title('RMSD SALINITY 10-30m MEDSEA 3rd DAY FORECAST' ,fontsize=18)
ax.xaxis.set_major_locator(mdates.YearLocator())
ax.xaxis.set_minor_locator(mdates.MonthLocator((1,4,7,10)))
ax.xaxis.set_major_formatter(mdates.DateFormatter("\n%Y"))
ax.xaxis.set_minor_formatter(mdates.DateFormatter("%b"))
plt.setp(ax.get_xticklabels(), rotation=0, ha="center")
plt.savefig(figdir+'SAL_RMSD_10-30_3RD_MED_ma.jpg')
##
...
leg.get_frame().set_alpha(0.3)
ax.grid('on')
plt.title('RMSD SEA LEVEL MEDSEA 3rd DAY FORECAST' ,fontsize=18)
ax.xaxis.set_major_locator(mdates.YearLocator())
ax.xaxis.set_minor_locator(mdates.MonthLocator((1,4,7,10)))
ax.xaxis.set_major_formatter(mdates.DateFormatter("\n%Y"))
ax.xaxis.set_minor_formatter(mdates.DateFormatter("%b"))
plt.setp(ax.get_xticklabels(), rotation=0, ha="center")
plt.savefig(figdir+'SLA_RMSD_3RD_MED_ma.jpg')
##
...
leg.get_frame().set_alpha(0.3)
ax.grid('on')
plt.title('RMSD SST MEDSEA 3rd DAY FORECAST' ,fontsize=18)
ax.xaxis.set_major_locator(mdates.YearLocator())
ax.xaxis.set_minor_locator(mdates.MonthLocator((1,4,7,10)))
ax.xaxis.set_major_formatter(mdates.DateFormatter("\n%Y"))
ax.xaxis.set_minor_formatter(mdates.DateFormatter("%b"))
plt.setp(ax.get_xticklabels(), rotation=0, ha="center")
plt.savefig(figdir+'SST_RMSD_3RD_MED_ma.jpg')
#
