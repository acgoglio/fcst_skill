...
#        RMS_T16=np.sqrt(np.true_divide(TEMPERATURE16[:,0,0,3,0],TEMPERATURE16[:,0,0,0,0], where=(TEMPERATURE16[:,0,0,0,0]!=0)))
        RMS_T16=np.sqrt(TEMPERATURE16[:,3,0,3,0], where=(TEMPERATURE16[:,0,0,0,0]!=0))
        STAT_T16.extend(RMS_T16)
        RMS_S16=np.sqrt(SALINITY16[:,3,0,3,0], where=(SALINITY16[:,0,0,0,0]!=0))
        ...
        OBS_T16.extend(TEMPERATURE16[:,3,0,0,0])
        OBS_S16.extend(SALINITY16[:,3,0,0,0])
...
#Evaluates the time period
          ...
          RMS_T24=np.sqrt(TEMPERATURE24[:,3,0,3,0], where=(TEMPERATURE24[:,0,0,0,0]!=0))
          STAT_T24.extend(RMS_T24)
          RMS_S24=np.sqrt(SALINITY24[:,3,0,3,0], where=(SALINITY24[:,0,0,0,0]!=0))
          ...
          OBS_T24.extend(TEMPERATURE24[:,3,0,0,0])
          OBS_S24.extend(SALINITY24[:,3,0,0,0])
...
ax.grid('on')
plt.title('RMSD TEMPERATURE 0-10m MEDSEA 3rd DAY FORECAST' ,fontsize=18)
ax.set_ylim([0.2,1.6])
...
plt.savefig(figdir+'TEMP_RMSD_SURF_3RD_MED_ma.jpg')
##
...
leg.get_frame().set_alpha(0.3)
ax.grid('on')
plt.title('RMSD SALINITY 0-10m MEDSEA 3rd DAY FORECAST' ,fontsize=18)
ax.xaxis.set_major_locator(mdates.YearLocator())
ax.xaxis.set_minor_locator(mdates.MonthLocator((1,4,7,10)))
ax.xaxis.set_major_formatter(mdates.DateFormatter("\n%Y"))
ax.xaxis.set_minor_formatter(mdates.DateFormatter("%b"))
plt.setp(ax.get_xticklabels(), rotation=0, ha="center")
plt.savefig(figdir+'SAL_RMSD_SURF_3RD_MED_ma.jpg')
##
